...
\begin{parts}
    ...
    \[
    \sum_{j=1}^{n} l_j\, x_{j} \leq L, \quad x_{j} \in \mathbb{Z}_+, \quad x \in P \quad \Rightarrow{} \quad x \in X.
    \]
    We can formulate this as follow: \begin{equation*}
        X=\bigg\{ x\in \mathbb{Z}^n_+ :  \sum_{j=1}^{n} l_j\, x_{j} \leq L \bigg\}
    \end{equation*}
    \end{solutionorbox}
    ...
    Let \(X\) denote the set of feasible cutting patterns \(x^t \in P\), where \(x^t_j\) is the number of items of type \(j\) in pattern \(t\) and \(y_t \in \mathbb{Z}_+\) represents the number of boards cut according to pattern \(t\).
    \[
    \begin{aligned}
    \text{(P1)}\quad 
    \underset{y_t \in \mathbb{Z}_+}{\min}\;& \sum_{t\in X} y_t\\
    \text{s.t.}\;& \sum_{t\in X} x^t_{j}\,y_t \;\ge\; d_j, && j=1,\ldots,n,\; \text{and }x^ t\in X.
    \end{aligned}
    \]
    or in matrix form with $y\in Z_+^{T}$,  $A\in Z_+^{ n\times T}$ such that $a_{jt}=x^t_j$,  $|X|=T$ and $d =  \
    [d_1, \cdots, d_n]$ we obtain:
    \[
    \begin{aligned}
    \text{(P1)}\quad 
    \underset{y \in \mathbb{Z}^T_+}{\min}\;&  \mathds{1}^\top y \\
    ...
    \begin{equation*}
        X = \bigg\{x \in \mathbb{Z}_+^n \Big|  \sum_{j=1}^n x_j \leq n, \quad x_j \in \mathbb{Z}_+\bigg\}
    \end{equation*}
